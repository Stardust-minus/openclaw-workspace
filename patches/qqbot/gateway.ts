...
import { analyzeImageWithVision } from "./utils/vision-analyze.js";
    ...
    try {
      ...
      const handleMessage = async (event: {
          ...
          for (const att of event.attachments) {
            ...
            if (localPath) {
              if (att.content_type?.startsWith("image/")) {
                ...
                // 自动分析图片内容
                log?.info(`[qqbot:${account.accountId}] Analyzing image with vision API: ${localPath}`);
                try {
                  const visionResult = await analyzeImageWithVision(localPath, "请详细描述这张图片的内容");
                  if (visionResult) {
                    const shortDesc = visionResult.length > 300 ? visionResult.slice(0, 300) + "..." : visionResult;
                    imageDescriptions.push(`[图片内容] ${shortDesc}`);
                    log?.info(`[qqbot:${account.accountId}] Vision analysis completed`);
                  }
                } catch (visionErr) {
                  log?.error(`[qqbot:${account.accountId}] Vision analysis failed: ${visionErr}`);
                }
              } else if (isVoice) {
        ...
        if (imageDescriptions.length > 0) {
          imageText = imageDescriptions.join("\n") + "\n";
        }
...
`;
        ...
        // 注意：需要注入图片描述（imageText），否则 AI 看不到图片内容
